...
public:
  ...
  auto makeTcpClient() -> brilliant::snapcast::TcpClient<FakeSocket<boost::asio::ip::tcp>> {
    return {FakeSocket<boost::asio::ip::tcp>(context.get_executor(), &socketState), mr};
  }
...
TEST_F(TestTcpClient, testConnect) {
  ...
  boost::asio::co_spawn(context, [this, &tcpClient] -> boost::asio::awaitable<void> {
...
TEST_F(TestTcpClient, testRead) {
  ...
  boost::asio::co_spawn(context, [this, &tcpClient] -> boost::asio::awaitable<void> {
    ...
    auto [ec, size] = co_await tcpClient.read(std::span(buffer.data(), data.size()));
    EXPECT_EQ(ec, boost::asio::error::operation_aborted);

    socketState.ec = boost::system::error_code{};
    std::tie(ec, size) = co_await tcpClient.read(std::span(buffer.data(), data.size()));
    EXPECT_FALSE(ec);
    EXPECT_EQ(size, data.size());
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(data, std::string_view(reinterpret_cast<const char*>(buffer.data()), size));
  }, boost::asio::detached);
  context.run();
}
...
TEST_F(TestTcpClient, testWrite) {
  ...
  boost::asio::co_spawn(context, [this, &tcpClient] -> boost::asio::awaitable<void> {
    ...
    auto [ec, size] = co_await tcpClient.write(std::span(buffer.data(), data.size()));
    EXPECT_EQ(ec, boost::asio::error::no_such_device);

    socketState.ec = boost::system::error_code{};
    std::tie(ec, size) = co_await tcpClient.write(std::span(buffer.data(), data.size()));
    EXPECT_FALSE(ec);
    EXPECT_EQ(size, data.size());
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    EXPECT_EQ(data, std::string_view(reinterpret_cast<const char*>(socketState.outData.data()), size));
  }, boost::asio::detached);
  context.run();
}
